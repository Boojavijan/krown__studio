...
const timberSectionTextLines = new SplitText(
  document.querySelector(".timber .text-block p"),
  { type: "lines" }
).lines;
const heroSubtitle = document.querySelector(".hero .title-block .sub-title");
const heroTitleLines = new SplitText(
  document.querySelector(".hero .title-block .title-h1"),
  { type: "lines" }
).lines;
const heroDescLines = new SplitText(
  document.querySelector(".hero .text-block .desc-1"),
  { type: "lines" }
).lines;
...
document.addEventListener("DOMContentLoaded", () => {
  gsap.set([heroSubtitle, heroTitleLines], { autoAlpha: 0, y: 50 });
  ...
  heroSectionSecondTimeline
    ...
    .fromTo(
      heroDescSubtitle,
      { autoAlpha: 0, y: 50 },
      { autoAlpha: 1, y: 0, duration: 0.833, ease: "power1.inOut" },
      0.0666
    )
    .fromTo(
      heroDescLines,
  ...
  timberSectionPhotoBlocks.forEach((el, index) => {
    ...
    timberSectionTimeline
      .to(cols, { width: 0, duration: 1, ease: "power1.inOut" }, delay)
      .to(
        photo,
        {
          top: index === 1 ? -100 : -210,
  ...
  createScrollTrigger(".hero .text-block", () =>
    activateTimeline(heroSectionSecondTimeline)
  );
  ...
  window.addEventListener("load", async () => {
    ...
    gsap.to(heroImageCols, { width: 0, duration: 1, ease: "power1.inOut" }, 0.2);
    gsap.to(
      heroPhoto,
      { top: "-20%", duration: 1.33333, ease: "power1.inOut" },
      0.3
    );
    gsap.to(
      heroSubtitle,
